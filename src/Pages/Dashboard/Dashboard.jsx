import React from 'react'
import department from '../../assets/department.png'
import job from '../../assets/job.png'
import { DashboardNav } from '../../Components/DashboardNav'
import { NavLink, Outlet, useNavigate } from 'react-router'
import employee from '../../assets/Employee.png'
import assets from '../../assets/assts.png'
import recruit from '../../assets/recruit.png'
import leave from '../../assets/leave.png'
import overview from '../../assets/images.png'

export const Dashboard = () => {

     const navigate = useNavigate();
    
      // const handleSelect = (e) => {
      //   const value = e.target.value;
      //   if (value) navigate(value); 
      // };
    
  return (
   <div className='w-full min-h-screen bg-blue-100'>
    <DashboardNav></DashboardNav>
     <div className=' flex'>
      <div className='w-72 min-h-screen bg-linear-to-b from-[#9f8ff7] to-sky-600 font-bold'>
         <div className="relative m-4 w-20">
				<span className="absolute inset-y-0 left-0 flex items-center pl-2">
					<button type="submit" title="Search" className="p-1 focus:outline-none focus:ring">
						<svg fill="currentColor" viewBox="0 0 512 512" className="w-4 h-4 dark:text-gray-100">
							<path d="M479.6,399.716l-81.084-81.084-62.368-25.767A175.014,175.014,0,0,0,368,192c0-97.047-78.953-176-176-176S16,94.953,16,192,94.953,368,192,368a175.034,175.034,0,0,0,101.619-32.377l25.7,62.2L400.4,478.911a56,56,0,1,0,79.2-79.195ZM48,192c0-79.4,64.6-144,144-144s144,64.6,144,144S271.4,336,192,336,48,271.4,48,192ZM456.971,456.284a24.028,24.028,0,0,1-33.942,0l-76.572-76.572-23.894-57.835L380.4,345.771l76.573,76.572A24.028,24.028,0,0,1,456.971,456.284Z"></path>
						</svg>
					</button>
				</span>
				<input type="search" name="Search" placeholder="Search menu..." className="w-60 m py-2 pl-10 text-sm  focus:outline-none  bg-[#FFFFFF99] border rounded-lg" />
			</div>

              <NavLink to='overview' className={({ isActive }) => 
    `flex px-6 w-60 mx-4 items-center py-3 rounded-lg gap-3 ${isActive ? 'bg-[#01010133]' : 'hover:bg-[#01010133]'}`
  }> 
                 <ul className='flex items-center gap-3'>
                  <img className='w-8 h-8 text-black' src={overview} alt="" />

                    <li>Overview</li>
                  </ul></NavLink>    
        <NavLink to='menu' className={({ isActive }) => 
    `flex px-6 w-60 mx-4 items-center py-3 rounded-lg gap-3 ${isActive ? 'bg-[#01010133]' : 'hover:bg-[#01010133]'}`
  }> 
                 <ul className='flex items-center gap-3'>
                  <img className='w-8 h-8' src={department} alt="" />

                    <li>Department</li>
                  </ul></NavLink>
        <NavLink to='job' className={({ isActive }) => 
    `flex px-6 w-60 mx-4 items-center my-2 py-3 rounded-lg gap-3 ${isActive ? 'bg-[#01010133]' : 'hover:bg-[#01010133]'}`
  }> 
                 <ul className='flex items-center gap-3'>
                  <img className='w-10 h-10' src={job} alt="" />

                    <li>Jobs</li>
                  </ul></NavLink>
        <NavLink to='employee' className={({ isActive }) => 
    `flex px-6 w-60 mx-4 items-center my-2 py-3 rounded-lg gap-3 ${isActive ? 'bg-[#01010133]' : 'hover:bg-[#01010133]'}`
  }> 
                 <ul className='flex items-center gap-3'>
                  <img className='w-8 h-8' src={employee} alt="" />

                    <li>Employee</li>
                  </ul></NavLink>
        <NavLink to='Assets' className={({ isActive }) => 
    `flex px-6 w-60 mx-4 items-center my-2 py-3 rounded-lg gap-3 ${isActive ? 'bg-[#01010133]' : 'hover:bg-[#01010133]'}`
  }> 
                 <ul className='flex items-center gap-3'>
                  <img className='w-8 h-8' src={assets} alt="" />

                    <li>Assets</li>
                  </ul></NavLink>
        <NavLink to='Recruitment' className={({ isActive }) => 
    `flex px-6 w-60 mx-4 items-center my-2 py-3 rounded-lg gap-3 ${isActive ? 'bg-[#01010133]' : 'hover:bg-[#01010133]'}`
  }> 
                 <ul className='flex items-center gap-3'>
                   <img className='w-8 h-8 ' src={recruit} alt="" />

                    <li>Recruitment</li>
                  </ul></NavLink>
        <NavLink to='leave' className={({ isActive }) => 
    `flex px-6 w-60 mx-4 items-center my-2 py-3 rounded-lg gap-3 ${isActive ? 'bg-[#01010133]' : 'hover:bg-[#01010133]'}`
  }> 
                
                    
             <ul className='flex items-center gap-3'>
                   <img className='w-8 h-8' src={leave} alt="" />

                          <li>Leave</li>
             
                  </ul>
                  </NavLink>
        <NavLink to='/' className={({ isActive }) => 
    `flex px-6 w-60 mx-4 items-center my-2 py-3 rounded-lg gap-3 ${isActive ? 'bg-[#01010133]' : 'hover:bg-[#01010133]'}`
  }> 
                
                    
             <ul className='flex items-center gap-3'>
                   <img className='w-6 h-6' src='https://www.svgrepo.com/show/493950/home-home.svg' alt="" />

                          <li>Back to Home</li>
             
                  </ul>
                  </NavLink>
                
      </div>

    <div>
       <Outlet></Outlet>
    </div>


    </div>
   </div>
  )
}
